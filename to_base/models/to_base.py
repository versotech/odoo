...
class TOBase(models.AbstractModel):
    ...
    def find_last_date_of_period(self, period_name, date, date_is_start_date=False):
        ...
        if period_name == 'weekly':
            ...
            dt = fields.Date.from_string('%s-%s-%s' % (date.year, date.month, days_of_month))
        elif period_name == 'quarterly':
            if date.month >= 1 and date.month <= 3:
                dt = fields.Date.from_string('%s-%s-%s' % (date.year, 3, 31))
            elif date.month >= 4 and date.month <= 6:
                dt = fields.Date.from_string('%s-%s-%s' % (date.year, 6, 30))
            elif date.month >= 7 and date.month <= 9:
                dt = fields.Date.from_string('%s-%s-%s' % (date.year, 9, 30))
            else:
                dt = fields.Date.from_string('%s-%s-%s' % (date.year, 12, 31))
        elif period_name == 'biannually':
            if date.month <= 6:
                dt = fields.Date.from_string('%s-%s-%s' % (date.year, 6, 30))
            else:
                dt = fields.Date.from_string('%s-12-31' % date.year)
        else:
            dt = fields.Date.from_string('%s-12-31' % date.year)
        return dt
...
#         print(res)
    ...
    def get_host_ip(self):
        ...
        This method relies on an external service which is https://ipapi.co/ and could be down a day. Use it at your own risk!
        """
        # TODO: implement our own mechanism instead of using the https://ipapi.co/
